...
pub struct AnalysisReport {
    ...
    pub packages_with_inconsistencies: usize,
...
impl TourneeResponse {
    ...
    pub fn analyze_geocoding(&self) -> AnalysisReport {
        let mut report = AnalysisReport {
            ...
            packages_with_inconsistencies: 0,
            quality_distribution: HashMap::new(),
            algorithm_distribution: HashMap::new(),
            recommendation_distribution: HashMap::new(),
        ...
        for package in &self.lst_lieu_article {
            ...
            if analysis.has_inconsistencies {
                report.packages_with_inconsistencies += 1;
            }
            ...
            let rec_str = match analysis.recommendation {
                Recommendation::UseExisting => "UseExisting",
                Recommendation::ValidateCoords => "ValidateCoords",
                Recommendation::ReGeocode => "ReGeocode",
                Recommendation::ManualReview => "ManualReview",
            };
            *report.recommendation_distribution.entry(rec_str.to_string()).or_insert(0) += 1;
...
impl AnalysisReport {
    pub fn print_summary(&self) {
        println!("\n{}", "=".repeat(50));
        println!("ANÁLISIS DE GEOCODING - COLIS PRIVÉ");
        println!("{}", "=".repeat(50));
        ...
        println!("  ❓ Desconocido: {} ({:.1}%)", 
                self.geocoding_unknown, 
                (self.geocoding_unknown as f64 / self.total_packages as f64) * 100.0);
        ...
        println!("  ⚠️  Con inconsistencias: {} ({:.1}%)", 
                self.packages_with_inconsistencies, 
                (self.packages_with_inconsistencies as f64 / self.total_packages as f64) * 100.0);
        ...
        for (rec, count) in &self.recommendation_distribution {
            println!("  {}: {} ({:.1}%)", 
                    rec, 
                    count, 
                    (*count as f64 / self.total_packages as f64) * 100.0);
        ...
        println!("\n🔍 Ejemplos de Paquetes Problemáticos:");
        let problem_packages: Vec<_> = self.recommendations.iter()
            .filter(|r| matches!(r.status, GeocodingStatus::Failed | GeocodingStatus::Partial) || r.has_inconsistencies)
            .take(5)
            .collect();
        
        for (i, pkg) in problem_packages.iter().enumerate() {
            println!("  {}. {}: {:?} - {:?}", i+1, pkg.package_id, pkg.status, pkg.recommendation);
            if pkg.has_inconsistencies {
                println!("     ⚠️  Inconsistencias detectadas");
            }
            if !pkg.missing_fields.is_empty() {
                println!("     ❌ Campos faltantes: {}", pkg.missing_fields.join(", "));
            }
        }
        
        if self.recommendations.len() > 5 {
            let problem_count = self.recommendations.iter()
                .filter(|r| matches!(r.status, GeocodingStatus::Failed | GeocodingStatus::Partial) || r.has_inconsistencies)
                .count();
            if problem_count > 5 {
                println!("     ... y {} paquetes más con problemas", problem_count - 5);
            }
        }
        
        // Mostrar optimización potencial
        let usable_packages = self.packages_with_coordinates; // Ya incluye los completos
        println!("\n🚀 OPTIMIZACIÓN POTENCIAL:");
        println!("  📊 Paquetes listos para usar: {} ({:.1}%)", 
                usable_packages, 
                (usable_packages as f64 / self.total_packages as f64) * 100.0);
        println!("  🔄 Paquetes que necesitan re-geocoding: {} ({:.1}%)", 
                self.geocoding_failed, 
                (self.geocoding_failed as f64 / self.total_packages as f64) * 100.0);
        println!("  ⚡ Reducción de trabajo: {:.1}% de paquetes ya procesados", 
                (usable_packages as f64 / self.total_packages as f64) * 100.0);
    }
}
